...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function moveLavalamp() {
      ...
      if (currentItem) {
        const itemWidth = currentItem.parentElement.offsetWidth;
        const itemOffset = currentItem.parentElement.offsetLeft;
        const left = itemOffset + itemWidth / 2 - lavalamp.offsetWidth / 2;
        lavalamp.style.left = left + 'px';
      }
    }
    ...
    menuItems.forEach(item => {
      item.addEventListener('click', () => {
        ...
        menuItems.forEach(menuItem => menuItem.querySelector('a').classList.remove('active'));
  
     
        item.querySelector('a').classList.add('active');
  

        const selectedType = item.querySelector('a').textContent;
